#!/usr/bin/env bash
# Bind eligibility proof to CompliancePolicy via LumengateSmartAccount (delegated admin auth).
set -euo pipefail
...
[[ -n "$SMART" && -n "$CPOL" ]] || { echo "Smart account or compliance policy missing"; exit 1; }
