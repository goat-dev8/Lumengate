...
import { hash, Keypair, TransactionBuilder, rpc, contract, } from "@stellar/stellar-sdk";
...
import { IndexerClient, DEFAULT_INDEXER_URLS, } from "./indexer";
...
import { discoverContractsByCredential, discoverContractsByAddress, getContractDetailsFromIndexer, } from "./kit/indexer-ops";
import { createPasskey, authenticatePasskey, signAuthEntry, } from "./kit/webauthn-ops";
import { createWallet, connectWallet, connectWithCredentials, disconnect, } from "./kit/wallet-ops";
import { buildDeployTransaction, submitDeploymentTx, } from "./kit/deploy-ops";
import { sign, signAndSubmit, fundWallet, buildTokenTransferTargetArgs, hasSourceAccountAuth, signResimulateAndPrepare, shouldUseFeeSponsoring, sendAndPoll, } from "./kit/tx-ops";
import { convertPolicyParams, buildPoliciesScVal } from "./kit/policies-ops";
import { findWebAuthnSignerForCredential, resolveContextRuleIdsForEntry, } from "./kit/context-rules";
...
export class SmartAccountKit {
    ...
    constructor(config) {
        ...
        this.deployerKeypair = Keypair.fromRawEd25519Seed(hash(Buffer.from("openzeppelin-smart-account-kit")));
    ...
    async getActiveContractDetailsFromIndexer() {
        if (!this._contractId) {
            return null;
        }
    ...
    async sign(transaction, options) {
        const resolvedOptions = {
            ...options,
            resolveContextRuleIds: options?.resolveContextRuleIds ?? ((entry) => this.resolveConnectedContextRuleIds(entry, options?.credentialId)),
        };
    ...
    async transfer(tokenContract, recipient, amount, options) {
        ...
        try {
            ...
            return this.signAndSubmit(transaction, {
                ...
                resolveContextRuleIds: (entry) => this.resolveConnectedContextRuleIds(entry, options?.credentialId),
     ...
     */
    async execute(target, targetFn, targetArgs) {
        const { wallet } = this.requireWallet();
     ...
     * @returns Transaction result
     */
    async executeAndSubmit(target, targetFn, targetArgs, options) {
    ...
    async resolveConnectedContextRuleIds(entry, credentialIdOverride) {
