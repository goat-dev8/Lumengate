...
import { SECP256R1_PUBLIC_KEY_SIZE, UNCOMPRESSED_PUBKEY_PREFIX, STROOPS_PER_XLM, } from "./constants";
...
export function validateNotEmpty(value, fieldName) {
    if (!value || typeof value !== "string" || value.trim().length === 0) {
        throw new ValidationError(`${fieldName} is required`, SmartAccountErrorCode.INVALID_INPUT, { field: fieldName });
    }
}
// ============================================================================
// Conversion Helpers
 ...
 */
export function buildKeyData(publicKey, credentialId) {
 ...
 */
export function extractPubkeyFromKeyData(keyData) {
 ...
 */
export function extractCredentialIdFromKeyData(keyData) {
 ...
 */
export function deriveContractAddress(credentialId, deployerPublicKey, networkPassphrase) {
    ...
    if (response.publicKey) {
        ...
        if (encodedPublicKey.length === SECP256R1_PUBLIC_KEY_SIZE &&
            ...
            try {
                ...
                publicKey = encodedPublicKey.slice(encodedPublicKey.length - SECP256R1_PUBLIC_KEY_SIZE);
            }
        }
        else {
            publicKey = encodedPublicKey.slice(encodedPublicKey.length - SECP256R1_PUBLIC_KEY_SIZE);
