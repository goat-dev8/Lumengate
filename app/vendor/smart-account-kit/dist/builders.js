 ...
 * ```
 */
export function createDelegatedSigner(publicKey) {
    // Validate the address format
    if (!publicKey.startsWith("G") || publicKey.length !== 56) {
        throw new ValidationError("Invalid Stellar account address. Must start with 'G' and be 56 characters.", SmartAccountErrorCode.INVALID_ADDRESS, { publicKey });
    }
    return {
        tag: "Delegated",
        values: [publicKey],
 ...
 * ```
 */
export function createExternalSigner(verifierAddress, keyData) {
    // Validate the address format
    if (!verifierAddress.startsWith("C") || verifierAddress.length !== 56) {
        throw new ValidationError("Invalid contract address. Must start with 'C' and be 56 characters.", SmartAccountErrorCode.INVALID_ADDRESS, { verifierAddress });
    }
    return {
        tag: "External",
 ...
 * ```
 */
export function createWebAuthnSigner(webauthnVerifierAddress, publicKey, credentialId) {
 ...
 * ```
 */
export function createEd25519Signer(ed25519VerifierAddress, publicKey) {
    if (publicKey.length !== 32) {
        throw new ValidationError("Ed25519 public key must be 32 bytes", SmartAccountErrorCode.INVALID_INPUT, { actualLength: publicKey.length });
    }
 ...
 * ```
 */
export function createCallContractContext(contractAddress) {
    // Validate the address format
    if (!contractAddress.startsWith("C") || contractAddress.length !== 56) {
        throw new ValidationError("Invalid contract address. Must start with 'C' and be 56 characters.", SmartAccountErrorCode.INVALID_ADDRESS, { contractAddress });
    }
    return {
        tag: "CallContract",
 ...
 * ```
 */
export function createCreateContractContext(wasmHash) {
    let hashBuffer;
    if (typeof wasmHash === "string") {
        // Remove 0x prefix if present
        const cleanHash = wasmHash.startsWith("0x") ? wasmHash.slice(2) : wasmHash;
        if (cleanHash.length !== 64) {
            throw new ValidationError("WASM hash must be 32 bytes (64 hex characters)", SmartAccountErrorCode.INVALID_INPUT, { actualLength: cleanHash.length });
        }
        hashBuffer = Buffer.from(cleanHash, "hex");
    }
    else {
        if (wasmHash.length !== 32) {
            throw new ValidationError("WASM hash must be 32 bytes", SmartAccountErrorCode.INVALID_INPUT, { actualLength: wasmHash.length });
        }
 ...
 * ```
 */
export function createThresholdParams(threshold) {
    if (threshold < 1) {
        throw new ValidationError("Threshold must be at least 1", SmartAccountErrorCode.INVALID_INPUT, { threshold });
    }
    if (!Number.isInteger(threshold)) {
        throw new ValidationError("Threshold must be an integer", SmartAccountErrorCode.INVALID_INPUT, { threshold });
    }
    return { threshold };
}
/**
 ...
 * ```
 */
export function createWeightedThresholdParams(threshold, signerWeights) {
    if (threshold < 1) {
        throw new ValidationError("Threshold must be at least 1", SmartAccountErrorCode.INVALID_INPUT, { threshold });
    }
    if (!Number.isInteger(threshold)) {
    ...
    if (signerWeights.size === 0) {
        throw new ValidationError("At least one signer weight must be provided", SmartAccountErrorCode.INVALID_INPUT);
    }
    ...
    for (const [, weight] of signerWeights) {
        if (weight < 1 || !Number.isInteger(weight)) {
            throw new ValidationError("All weights must be positive integers", SmartAccountErrorCode.INVALID_INPUT, { weight });
        }
        totalWeight += weight;
    }
    // Validate that threshold is achievable
    if (totalWeight < threshold) {
        throw new ValidationError(`Sum of weights (${totalWeight}) must be >= threshold (${threshold})`, SmartAccountErrorCode.INVALID_INPUT, { threshold, totalWeight });
    }
 ...
 * ```
 */
export function createSpendingLimitParams(spendingLimit, periodLedgers) {
    if (periodLedgers < 1) {
        throw new ValidationError("Period must be at least 1 ledger", SmartAccountErrorCode.INVALID_INPUT, { periodLedgers });
    }
    if (!Number.isInteger(periodLedgers)) {
        throw new ValidationError("Period must be an integer number of ledgers", SmartAccountErrorCode.INVALID_INPUT, { periodLedgers });
    }
    const limitBigInt = typeof spendingLimit === "bigint" ? spendingLimit : BigInt(spendingLimit);
    if (limitBigInt < 1n) {
        throw new ValidationError("Spending limit must be positive", SmartAccountErrorCode.INVALID_AMOUNT, { spendingLimit: limitBigInt.toString() });
    }
    // Fields must be in alphabetical order for Soroban ScMap serialization
    return {
...
export function signerMatchesCredential(signer, credentialId) {
    return getCredentialIdFromSigner(signer) === credentialId;
}
export function signerMatchesAddress(signer, address) {
    return signer.tag === "Delegated" && signer.values[0] === address;
}
export function describeSignerType(signer) {
...
export function formatSignerForDisplay(signer) {
    if (signer.tag === "Delegated") {
        return {
            type: "G-Address",
...
export function formatContextType(contextType) {
