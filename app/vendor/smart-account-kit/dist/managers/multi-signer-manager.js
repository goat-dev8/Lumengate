 ...
 */
import { Address, hash, Operation, TransactionBuilder, xdr, rpc as rpcModule, } from "@stellar/stellar-sdk";
const { assembleTransaction } = rpcModule;
import { BASE_FEE, AUTH_ENTRY_EXPIRATION_BUFFER, } from "../constants";
import { getCredentialIdFromSigner, collectUniqueSigners, } from "../signer-utils";
import { buildAuthDigest, buildAddressSignatureScVal, buildSignaturePayload, readAuthPayload, upsertAuthPayloadSigner, writeAuthPayload, } from "../kit/auth-payload";
...
export class MultiSignerManager {
    ...
    async signWalletAddressAuthEntry(entry, authAddress, expiration) {
        ...
        const { signedAuthEntry } = await this.deps.externalSigners.signAuthEntry(preimage.toXDR("base64"), authAddress);
        signedEntry.credentials().address().signature(buildAddressSignatureScVal(Address.fromString(authAddress).toScAddress().accountId().ed25519(), Buffer.from(signedAuthEntry, "base64")));
        return signedEntry;
    }
    async submitWithSelectedSigners(hostFunc, authEntries, selectedSigners, options) {
        ...
        try {
            ...
            for (const [authEntryIndex, entry] of authEntries.entries()) {
                ...
                if (authAddress !== contractId) {
                    ...
                    signedAuthEntries.push(await this.signWalletAddressAuthEntry(entry, authAddress, expiration));
                    continue;
                }
                ...
                const resolvedContextRuleIds = options?.resolveContextRuleIds
                    ...
                    : await resolveContextRuleIdsForEntry(this.deps.requireWallet().wallet, signedEntry, selectedContractSigners, {
                ...
                for (const walletSigner of walletSigners) {
                    if (!walletSigner.walletAddress)
                        continue;
                    upsertAuthPayloadSigner(authPayload, walletSigner.signer, Buffer.alloc(0));
                }
                ...
                const signaturePayload = buildSignaturePayload(this.deps.networkPassphrase, signedEntry, expiration);
                ...
                for (const walletSigner of walletSigners) {
                    ...
                    signedAuthEntries.push(new xdr.SorobanAuthorizationEntry({
                        credentials: xdr.SorobanCredentials.sorobanCredentialsAddress(new xdr.SorobanAddressCredentials({
                            ...
                            nonce: delegatedNonce,
                            signatureExpirationLedger: expiration,
                            signature: buildAddressSignatureScVal(Address.fromString(walletSigner.walletAddress).toScAddress().accountId().ed25519(), Buffer.from(walletSignatureBase64, "base64")),
                        })),
            ...
            const normalizedTx = TransactionBuilder.fromXDR(resimTx.toXDR(), this.deps.networkPassphrase);
            const assembled = assembleTransaction(normalizedTx, resimResult);
            const preparedTx = assembled.build();
            const submissionOptions = { forceMethod: options?.forceMethod };
    ...
    async operation(assembledTx, selectedSigners, options) {
        ...
        try {
            ...
            const invokeOp = operations[0];
            const authEntries = invokeOp.auth || [];
            const submissionOptions = { forceMethod: options?.forceMethod };
            if (authEntries.length === 0) {
                onLog("No auth entries - submitting directly...");
                const preparedTx = TransactionBuilder.fromXDR(builtTx.toXDR(), this.deps.networkPassphrase);
                if (!this.deps.shouldUseFeeSponsoring(submissionOptions) || this.deps.hasSourceAccountAuth(preparedTx)) {
                    preparedTx.sign(this.deps.deployerKeypair);
                }
                return this.deps.sendAndPoll(preparedTx, submissionOptions);
            }
            return this.submitWithSelectedSigners(invokeOp.func, authEntries, selectedSigners, options);
        }
        catch (err) {
            return {
                success: false,
    ...
    async transfer(tokenContract, recipient, amount, selectedSigners, options) {
        ...
        try {
            ...
            return this.operation(assembledTx, selectedSigners, options);
        }
        catch (err) {
            return {
                success: false,
...
//# sourceMappingURL=multi-signer-manager.js.map