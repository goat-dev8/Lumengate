...
import { collectUniqueSigners, getCredentialIdFromSigner, signersEqual, } from "../signer-utils";
...
const READ_ONLY_SIM_ACCOUNT = new Account(Keypair.fromRawEd25519Seed(hash(Buffer.from("smart-account-kit-context-rule-read")))
    .publicKey(), "0");
export function contextRuleTypeKey(contextType) {
...
export function contextRuleTypeMatches(ruleType, requiredType) {
    return (ruleType.tag === "Default" ||
        contextRuleTypeKey(ruleType) === contextRuleTypeKey(requiredType));
}
export function buildInvocationContextTypes(entry) {
    const contexts = [];
...
function hasRpcReadConfig(deps) {
    return Boolean(deps?.rpc && deps.contractId && deps.networkPassphrase);
}
function decodeRequiredMapEntries(val) {
...
export function decodeContextRuleResultXdr(resultXdr) {
    ...
    return {
        context_type: decodeContextRuleType(entries.get("context_type")),
        id: expectScU32(entries.get("id"), "context rule id"),
        name: expectScString(entries.get("name"), "context rule name"),
        policies: expectScVec(policiesVal, "policies").map((policy, index) => expectScAddress(policy, `policy[${index}]`)),
...
async function hydrateContextRuleIds(wallet, rule) {
    ...
    const [policyIds, signerIds] = await Promise.all([
        needsPolicyIds
            ? wallet.get_policy_id
                ? Promise.all(rule.policies.map(async (policy) => {
        ...
        needsSignerIds
            ? wallet.get_signer_id
                ? Promise.all(rule.signers.map(async (signer) => {
...
async function readContextRuleFromRpc(contextRuleId, deps) {
...
export async function readContextRule(wallet, contextRuleId, deps) {
...
export async function listContextRules(wallet, deps) {
    const details = await deps?.getContractDetailsFromIndexer?.();
    const probeConfig = deps?.probeRuleIds;
    const discoveredRuleIds = new Set(details?.contextRules.map((rule) => rule.context_rule_id) ?? []);
    if (probeConfig) {
        const maxRuleId = probeConfig.maxRuleId ?? DEFAULT_MAX_PROBED_RULE_ID;
        const maxConsecutiveMisses = probeConfig.maxConsecutiveMisses ?? DEFAULT_MAX_CONSECUTIVE_PROBE_MISSES;
        let misses = 0;
    ...
    if (discoveredRuleIds.size === 0) {
        ...
        throw new Error("Listing active context rules requires the indexer because the contract does not expose an iterator for active rule IDs.");
    }
    const contextRuleIds = [...discoveredRuleIds].sort((a, b) => a - b);
    const rules = await Promise.all(contextRuleIds.map(async (contextRuleId) => {
        try {
...
export async function getFilteredContextRules(wallet, contextRuleType, deps) {
    const rules = await listContextRules(wallet, deps);
    return rules.filter((rule) => contextRuleTypeKey(rule.context_type) === contextRuleTypeKey(contextRuleType));
}
export async function findWebAuthnSignerInRules(wallet, contextRuleIds, credentialId, deps) {
    ...
    throw new Error(`No signer found for credential ID ${base64url.encode(Buffer.from(credentialId))} in context rules ${contextRuleIds.join(", ")}`);
}
export async function findWebAuthnSignerForCredential(wallet, credentialId, deps) {
    const rules = await listContextRules(wallet, deps);
    const matchingSigners = collectUniqueSigners(rules.flatMap((rule) => rule.signers.filter((signer) => getCredentialIdFromSigner(signer) === credentialId)));
    ...
    throw new Error(`Multiple WebAuthn signers matched credential ID ${credentialId}. Resolve the signer explicitly.`);
}
export async function resolveContextRuleIdsForEntry(wallet, entry, selectedSigners, deps) {
    const rules = await listContextRules(wallet, deps);
    ...
    return contexts.map((contextType) => {
        ...
        const exactSignerMatches = candidates.filter((rule) => {
            ...
            return (selectedSigners.every((selectedSigner) => rule.signers.some((ruleSigner) => signersEqual(ruleSigner, selectedSigner))) &&
                rule.signers.every((ruleSigner) => selectedSigners.some((selectedSigner) => signersEqual(ruleSigner, selectedSigner))));
        });
        ...
        const signerSubsetMatches = candidates.filter((rule) => {
            ...
            return rule.signers.every((ruleSigner) => selectedSigners.some((selectedSigner) => signersEqual(ruleSigner, selectedSigner)));
        });
        if (signerSubsetMatches.length === 1) {
            return signerSubsetMatches[0].id;
        }
        const ids = candidates.map((candidate) => candidate.id).join(", ");
        throw new Error(`Unable to resolve a unique context rule for ${contextRuleTypeKey(contextType)}. ` +
...
function isMissingContextRuleError(error) {
    if (!(error instanceof Error)) {
        return false;
    }
    return (/ContextRuleNotFound/i.test(error.message) ||
