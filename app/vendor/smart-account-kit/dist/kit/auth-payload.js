...
export function buildAuthDigest(signaturePayload, contextRuleIds) {
    const ruleIdsXdr = xdr.ScVal.scvVec(contextRuleIds.map((contextRuleId) => xdr.ScVal.scvU32(contextRuleId))).toXDR();
    return hash(Buffer.concat([signaturePayload, ruleIdsXdr]));
}
export function buildWebAuthnSignatureBytes(sigData) {
...
export function buildAddressSignatureScVal(publicKeyBytes, signatureBytes) {
...
export function emptyAuthPayload() {
    return {
        context_rule_ids: [],
        signers: new Map(),
    };
}
export function readAuthPayload(signature) {
...
export function writeAuthPayload(payload) {
    const signerEntries = Array.from(payload.signers.entries()).map(([signer, signatureBytes]) => new xdr.ScMapEntry({
    ...
    return xdr.ScVal.scvMap([
        new xdr.ScMapEntry({
            ...
            val: xdr.ScVal.scvVec(payload.context_rule_ids.map((contextRuleId) => xdr.ScVal.scvU32(contextRuleId))),
        }),
        new xdr.ScMapEntry({
...
export function upsertAuthPayloadSigner(payload, signer, signatureBytes) {
...
function parseSignerScVal(value) {
    if (value.switch().name !== "scvVec") {
